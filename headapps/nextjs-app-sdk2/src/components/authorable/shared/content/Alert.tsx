...
import type { SiteAlert } from 'lib/site-settings/types';
...
const useAlerts = (alertTypeGuid: string | undefined) => {
  ...
  const matchedAlerts = useMemo(
    () =>
      siteAlerts.filter((alert) => {
        const alertTypeGuidFromAlert = (alert?.alertType?.jsonValue as { id?: string } | undefined)
          ?.id;
        const expectedGuid = alertTypeGuid?.toLowerCase();
        const matchesType =
          !expectedGuid || alertTypeGuidFromAlert?.toLowerCase() === expectedGuid;
  ...
  return { matchedAlerts, state, dismissAlert, handleDismissed };
};
    ...
    <div className={`${container()} ${state.hydrated ? 'block' : 'hidden'}`}>
      {matchedAlerts.map((alert) => {
        const alertTypeValue =
          (
            alert.alertType?.jsonValue as { fields?: { title?: { value?: string } } } | undefined
          )?.fields?.title?.value?.toLowerCase() || DEFAULT_ALERT_TYPE;
        ...
        return (
          <div
            ...
            onTransitionEnd={() => {
              if (isFading && alert.id) handleDismissed(alert.id);
            }}
          >
...
const TAILWIND_VARIANTS = tv({
  ...
  variants: {
    type: {
      priority: { alertStyle: ['bg-component-alert-bg-priority'] },
      neutral: { alertStyle: ['bg-component-alert-bg-neutral'] },
    },
  },
});
